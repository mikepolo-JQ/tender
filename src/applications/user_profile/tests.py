...
class UserProfileTests(APITestCase):
    ...
    def test_profile_page(self):
        ...
        assert response.status_code == 200
        assert response.data['username'] == self.username
        assert response.data['avatar'] == f'https://{settings.AWS_S3_CUSTOM_DOMAIN}/media/avatar.png'
        ...
        response = self.client.patch(path=f'/api/profile/{user.id}/', data=patch_data, format='json')
